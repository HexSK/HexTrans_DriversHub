...
frontend = CTk()
frontend.geometry("1000x563")  # Set initial window size
frontend.title("HexTrans Drivershub")
frontend.resizable(1, 0)  # Allow horizontal resizing only
frontend.grid_columnconfigure(0, weight=1)
frontend.grid_rowconfigure(0, weight=1)
frontend.update_idletasks()
frontend.iconbitmap(resource_path('./assets/HEXTRANS.ico'))


# Load database connection parameters from config file
with open(resource_path("config.json"), "r") as cfg_file:
...
# Initialize Discord webhook for job notifications
hook = Webhook(cfg["WEBHOOK_URL"])

# Session management functions
def save_session(email, username, steam_id):
    """Save user session data to a local JSON file"""
    session_data = {
        "email": email,
        "username": username,
        "steam_id": steam_id,
...
def load_session():
    """Load user session data from local JSON file if it exists"""
    session_file = Path("session.json")
...
def clear_session():
    """Remove the session file to log out user"""
    session_file = Path("session.json")
    if session_file.exists():
        os.remove(session_file)

def check_existing_session():
    """Check if user has a valid saved session and restore it"""
    ...
    if session_data and session_data.get("logged_in"):
        ...
        tabview.set("Jobs")  # Navigate to Jobs tab
        return True
    return False
...
def logout():
    ...
    tabview.set("Login")  # Return to login tab
        ...
        if logging_user is None:
            ...
            if check_password_hash(hashed_password, password):
                ...
                if steam_id_result:
                    ...
                    tabview.set("Jobs")
                    # Remove authentication tabs after successful login
...
def register():
    ...
    if username in users:
        ...
        # Verify passwords match and create new user account
...
# Define font styling for the application
h1 = ("Arial Bold", 36)
h2 = ("Arial Bold", 24)
font_entry = ("Arial", 20)
font_job = ("Arial", 18)

# Register tab UI components
...
# Jobs tab UI components
jobsTabFrame = CTkFrame(jobsTab)
jobsTabFrame.pack(fill=BOTH, expand=True)


def get_jobs():
    """Fetch and display user's delivery jobs from the database"""
    if steam_id:
        try:
            # Fetch jobs from DB for the current user
            cursor.execute(f"SELECT * FROM jobs WHERE steam_id = %s", (steam_id,))
            jobs = cursor.fetchall() 
            if jobs:
                # Create a scrollable frame if needed based on number of jobs
...
def submit_job():
    """Submit a new job to the database and send a Discord notification"""
    global steam_id
    steam_id = int(steam_id)
    job_id = random.randint(100000, 999999)  # Generate random job ID
    ...
    # Create and send Discord webhook notification
    embed = Embed(
        title="Job Submitted",
...
def load_profile():
    """Load and display user profile information from the database"""
    global steam_id
    ...
    if steam_id != "":
        ...
        # Update the load button to be a refresh button
...
if __name__ == "__main__":
    # Check for existing session and remove login/register tabs if logged in
